<section class="content-header">
    <h1>
        <?php echo ($user->id ? 'Edit User' : 'Create User'); ?>
        <small><?php echo ($user->id ? 'Update an Existing User' : 'Add a New User'); ?></small>
    </h1>
</section>
                ...
                <form action="<?php echo ($user->id ? '/users/edit/' . $user->id : '/users/add'); ?>" method="post" role="form">
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" maxlength="30" name="user[name]" class="form-control" value="<?php echo $user->name; ?>" required>
                        </div>
                        <div class="form-group">
                            <label>SSO <span class="required">*</span></label>
                            <input type="text" maxlength="32" name="user[username]" class="form-control" value="<?php echo $user->username; ?>" required>
                        </div>
                        <div class="form-group">
                            <label>Email <span class="required">*</span></label>
                            <input type="email" maxlength="254" name="user[email]" class="form-control" value="<?php echo $user->email; ?>" required>
                        </div>
                        <div class="form-group">
                            <label>Password <span class="required"><?php echo ($user->id ? '' : '*'); ?></span></label>
                            <input type="password" maxlength="64" name="user[password]" class="form-control" value="" <?php echo ($user->id ? '' : 'required'); ?>>
                        </div>
                        <div class="form-group">
                            <label>Department <span class="required"></span></label>
                            <select name="user[department_id]" class="form-control">
                    ...
                    <div class="box-footer">
                        <button type="submit" class="btn btn-primary"><?php echo ($user->id ? 'Update' : 'Create'); ?></button>
